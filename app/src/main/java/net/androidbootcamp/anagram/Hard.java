...
public class Hard extends AppCompatActivity
{
    ...
    int randomIndex = RANDOM.nextInt(HARDWORDS.length - 1);
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        scrambledWordView.setText(shuffleWord(HARDWORDS[randomIndex]));
        ...
        validateButton.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if(edText1.getText().toString().equals(HARDWORDS[randomIndex])){
                            SharedPreferences mPrefs = getSharedPreferences("score", counter);
                            SharedPreferences.Editor mEditor = mPrefs.edit();
                            mEditor.putInt("score", ++counter).commit();
                            Toast.makeText(net.androidbootcamp.anagram.Hard.this, "Score: " + mPrefs.getInt("score", counter) + "/4", Toast.LENGTH_LONG).show();
                            Intent intent = new Intent(getApplicationContext(), net.androidbootcamp.anagram.NewGame.class);
                            startActivity(intent);
                        }
                        else{
                            Toast.makeText(net.androidbootcamp.anagram.Hard.this, "Incorrect, try again", Toast.LENGTH_LONG).show();
                        }
                    }
                });
    }
